...
@RestController @RequestMapping("/api/cars") public class CarController { private final CarService service; public CarController(CarService s){service=s;}
 ...
 @GetMapping("/{id}") Object one(@PathVariable Long id){return service.get(id);} @PostMapping @PreAuthorize("hasRole('ADMIN')") ResponseEntity<?> create(@jakarta.validation.Valid @RequestBody CarService.CarCommand r){return ResponseEntity.status(201).body(service.save(null,r));}
 @PutMapping("/{id}") @PreAuthorize("hasRole('ADMIN')") Object update(@PathVariable Long id,@jakarta.validation.Valid @RequestBody CarService.CarCommand r){return service.save(id,r);} @DeleteMapping("/{id}") @PreAuthorize("hasRole('ADMIN')") ResponseEntity<?> delete(@PathVariable Long id){service.delete(id);return ResponseEntity.noContent().build();}
}
